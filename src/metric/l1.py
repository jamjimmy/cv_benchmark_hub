...
def preprocess_list(target_input, pred_input, device):
    ...
    for target_item, pred_item in tqdm(zip(target_input, pred_input), total=len(target_input)):
        ...
        processed_pred_input.append(torch.flatten(transform_img(pred_image).unsqueeze(0).to(device)))
        processed_target_input.append(torch.flatten(transform_img(target_image).unsqueeze(0).to(device)))
